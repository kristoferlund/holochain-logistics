...
const dnaPath = "dist/holochain-logistics.dna.json";
...
scenario.runTape(
  "Can register a profile and retrieve",
  async (t, { alice }) => {
    const register_result = await alice.callSync("event", "register", {
      name: "Food hub",
      avatar_url: "",
      description: "we are just around the corner"
    });
    console.log(register_result);
    t.true(register_result.Ok.includes("alice"));

    const get_profile_result = await alice.callSync(
      "event",
      "get_member_profile",
      { agent_address: register_result.Ok }
    );
...
scenario.runTape(
  "Can create some products and retrieve them",
  async (t, { alice }) => {
    const register_result = await alice.callSync("event", "register", {
      name: "Food hub",
      avatar_url: "",
    ...
    const create_product = await alice.callSync(
      "event",
      "create_product",
      testProduct
    );
    ...
    const create_product2 = await alice.callSync(
      "event",
      "create_product",
      testProduct2
    );
...
scenario.runTape(
  "Can create product, update it and retreive history",
  async (t, { alice }) => {
    const register_result = await alice.callSync("event", "register", {
      name: "Food hub",
      avatar_url: "",
    ...
    const testProductUpdate = Object.assign(testProduct, {
    ...
    const updated_product = await alice.call(
      "event",
      "update_product",
      testProductUpdate
    );
    ...
    const get_entry_history = await alice.callSync(
      "event",
      "get_entry_history",
      { entry_address: create_product.Ok }
    );
...
scenario.runTape(
  "Can create inventory items and retrieve them",
  async (t, { alice }) => {
    const register_result = await alice.callSync("event", "register", {
      name: "Food hub",
      avatar_url: "",
    ...
    const create_inventory = await alice.callSync("event", "create_inventory", {
      product_address: create_product.Ok,
      org_address: register_result.Ok,
      stocked_units: 100
    });
    console.log(create_inventory);
    t.deepEqual(create_inventory.Ok.length, 46);

    const create_inventory2 = await alice.callSync(
      "event",
      "create_inventory",
      {
...
//
scenario.runTape(
  "Can create create an order and update invetory of ordered product",
  async (t, { alice }) => {
    const register_result = await alice.callSync("event", "register", {
      name: "Food hub",
      avatar_url: "",
    ...
    const register_result2 = await alice.callSync("event", "register", {
      name: "Food hub 2",
      avatar_url: "",
      description: "we are a little further away"
    });
    ...
    const created_inventory = await alice.callSync(
      "event",
      "get_all_inventory",
      {}
    );
    ...
    const updated_inventory = await alice.callSync(
      "event",
      "get_all_inventory",
      {}
    );
...
*/
scenario.runTape(
  "Can post a message to the event and retrieve",
  async (t, { alice }) => {
    const register_result = await alice.callSync("event", "register", {
      name: "Food hub",
      avatar_url: "",
    ...
    const create_result = await alice.callSync(
      "event",
      "create_event",
      testNewChannelParams
    );
    console.log(create_result);
    const event_addr = create_result.Ok;
    t.deepEqual(event_addr.length, 46);

    const get_result = await alice.callSync(
      "event",
    ...
    const post_result = await alice.callSync("event", "post_message", {
    ...
    const get_message_result = await alice.callSync("event", "get_messages", {
      address: event_addr
    });
    console.log(get_message_result);
    t.deepEqual(
      get_message_result.Ok[0].entry.payload,
      testMessage.payload,
      "expected to receive the message back"
    );
  }
);

scenario.runTape(
  "Can create a public event with some members",
  async (t, { alice }) => {
    const register_result = await alice.callSync("event", "register", {
      name: "Food hub",
      avatar_url: "",
    ...
    const create_result = await alice.callSync("event", "create_event", {
      ...testNewChannelParams,
      public: false,
      initial_members: [register_result.Ok]
    });
    console.log(create_result);
    t.deepEqual(create_result.Ok.length, 46);

    const get_all_members_result = await alice.callSync(
      "event",
      "get_members",
      { event_address: create_result.Ok }
    );
    console.log("all members:", get_all_members_result);
